...
- Implemented in Wave 7 repository scope: immutable warm/Browser manifests; strict pre-candidate provider evidence schemas; guarded, injected protected HTTP and isolated proof-Blob probes; OIDC/static-token ambiguity rejection; provider gate derivation; a runtime-instance observation seam; and a provider attestation builder. These files and offline fixtures validate tooling only. They are not live Vercel, Blob, Browser, performance, memory, lifecycle, security, accounting, or attestation proof.
- Stewart separately authorized a later protected preview, but this Pi pass creates no provider or external resource. Hermes must independently review the artifact-only candidate and exact operator commands before executing or verifying any external side effect.
- Production promotion, website/domain cutover, paid-plan activation, Cloudflare, and Mac/tunnel operations remain outside Wave 7.
...
- 2026-07-20, Wave 7 repository tooling: strict protected-provider evidence, immutable manifests, guarded probes, derived provider gates, and provider-attestation construction. No fixture is live proof; every unavailable provider observation remains fail/blocked.
