...
- Implemented on the Wave 6 branch: strict canonical candidate identity with an exact 1 GiB ceiling and caller-supplied artifact topology, actual-policy and runtime configuration digests, a fixed all-or-nothing gate report, exact provider image-evidence binding, provider Deployment Attestation validation plus an explicit no-deployment local template, and manifest-triggered branch-independent exact-head candidate-container CI. Local/CI records remain ineligible while every protected/provider gate is blocked.
