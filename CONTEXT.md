...
- Supplied live proof for source `033cc01391ab3ca4db661e1a1837f57586249a43`, artifact `6a9f8f99c49cdbedf01327e050b451a2b144ca43`, bundle `ba180aed0e44202d01c5cc36786c04d140b9679b7248f5097a4c8f2955d5e982`, and deployment `dpl_A6LMSbEbfgRQqSJ7RS4TfKKgv7ke` confirmed the bounded cold-Browser wait but exposed a fifth repository blocker: both real UI submissions of built-in `40-40` reached the exact ten-second hard stop and provider POSTs returned 503. Independent review then rejected the runtime-built `/tmp` correction because same-UID writable Bundle/cache objects, path/hash/open races, lazy worker activation, and row-nonempty smoke evidence could false-pass. The corrected image now fully verifies the exact Bundle, Coverage Report, and canonical runtime as root at build time; atomically publishes one strict pointer and one referenced DuckDB/metadata object under fixed `/app/provider-runtime-cache`; finalizes Bundle, config, pointer, metadata, database, and parent directories as root-owned non-writable objects; and runs app and worker as dedicated UID/GID 10001. Provider startup and every worker eagerly validate the fixed pointer, source/Bundle/config identities, metadata hash, database size/hash, owner/mode/link/inode/ctime state, exact root/object contents, and exact read-only relation inventory; absence or drift never rebuilds or falls back. Fixed image paths now classify provider mode independently of the runtime-config environment, so missing, empty, foreign, symlinked, malformed, or hidden boundaries fail startup, health, and workers without local Bundle reconstruction. Canonical v3 smoke evidence preserves and validates the full observed worker payload with exact JSON scalar types: exact recipe, plan, rows, QueryEvidence, verification proof identity, counts, pagination, source fingerprints, SQL/bindings, and separate truthful image-preparation, activation-validation, and under-ten-second worker timing. Candidate parity gates semantically validate those bytes. Every retained cache construction, materialization, copy, lock, removal, write, and fsync entry point now requires literal effective UID 0 before reading inputs or mutating state; the candidate image directly probes all eight entry points as UID 10001 and requires exceptions with zero side effects. Local CI outside the fixed image boundary, ordinary development, and Release Proof retain full Bundle/runtime verification.
- Supplied follow-up provider evidence for exact deployment `dpl_9XW9KmE2rqe4XWZ7YBbmetEQLgab` exposed a sixth repository blocker: provider-evidence identity validation incorrectly sent the case-sensitive Vercel ID through the lowercase-only canonical-ID rule. Vercel deployment IDs now use a dedicated exact `dpl_` plus 28-character base62 validator across evidence, lifecycle replacement, probes, aggregation, and Deployment Attestation; no path normalizes or substitutes the provider value, and candidate-bound comparisons remain exact.
