...
- Supplied live proof for source `033cc01391ab3ca4db661e1a1837f57586249a43`, artifact `6a9f8f99c49cdbedf01327e050b451a2b144ca43`, bundle `ba180aed0e44202d01c5cc36786c04d140b9679b7248f5097a4c8f2955d5e982`, and deployment `dpl_A6LMSbEbfgRQqSJ7RS4TfKKgv7ke` confirmed the bounded cold-Browser wait but exposed a fifth repository blocker: both real UI submissions of built-in `40-40` reached the exact ten-second hard stop and provider POSTs returned 503. Protected initialization now fully verifies the exact Bundle and canonical in-memory runtime, atomically materializes an identity-bound read-only DuckDB cache under a fixed `/tmp` namespace, validates it, and only then publishes one digest reference to child workers. Protected children strictly validate source, Bundle, runtime configuration, metadata, ownership, modes, size, hash, and exact relation inventory before a read-only open; they never rebuild CSV tables or fingerprints and never fall back after cache readiness. Local CI, ordinary development, and Release Proof retain full verification.
