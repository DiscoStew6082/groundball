...
- The active map is stored on the canonical branch, not `main`. Resolve its checkout with `git worktree list --porcelain`; if the stable worktree is absent, recreate it using `docs/agents/issue-tracker.md`. Never switch the root worktree to the Wayfinder branch.
