...
- Stat explanation questions such as "what is OPS?" are answered directly by the local LLM rather than from Markdown or a vector index.
...
- "what is OPS" - open LLM explanation with no Markdown/vector-index lookup.
...
- Checked-in Markdown under `src/baseball_rag/corpus/` remains project material for docs/tests, but it is not runtime grounding for stat explanations or player biographies.
