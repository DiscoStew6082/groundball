...
- [Prove the stateless Svelte/FastAPI Vercel fit](issues/12-prove-stateless-svelte-fastapi-vercel-fit.md) — The unified 132.84 MB protected preview passed the server-enforced zero-Mac boundary, deterministic cold/default/fail-closed checks, two repeated `8/8 + 4/4` concurrent waves, packaged UI and export contracts, and final-preview cleanup. Vercel Hobby is the selected application shape; Cloudflare is ruled out of the current route and returns only as a fresh effort if this evidence is invalidated.
...
- [Define preview cutover and zero-Mac proof](issues/07-define-preview-cutover-and-zero-mac-proof.md) — Use the website launcher as the final reversible switch only after the attested Vercel candidate passes every gate and Stewart approves its link; abort on any miss, and reopen hosting only for a proven Vercel Hobby limitation.
