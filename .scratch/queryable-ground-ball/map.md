...
- [Specify the implementation handoff](issues/08-specify-implementation-handoff.md) — Build a clean seven-slice replacement from raw-query tracer through exhaustive proof, cut over the application without compatibility layers or fallback, and delete every superseded primary-Lahman query lane while leaving deployment responsibilities to the zero-Mac map.
