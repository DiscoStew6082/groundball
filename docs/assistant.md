...
This is a bounded agentic workflow: one model interpretation may select several approved operations, then the harness validates and executes them. It is not an autonomous research loop that feeds tool results back to the model. Correct source evidence alone does not prove that every condition in the user's wording survived interpretation. Scope guards and evaluations reduce that risk but do not eliminate it.
