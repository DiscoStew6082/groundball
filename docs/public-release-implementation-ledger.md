...
For provider runtime only, startup now validates local configuration and constructs the Blob Adapter without provider I/O before yielding. One process-wide background initializer then runs full Release Bundle/DuckDB readiness and startup-fallback Blob readiness exactly once. `GET /health` is `503 {"status":"initializing"}` until success, permanently becomes fixed sanitized `503 {"status":"failed"}` on error, and becomes `200 {"status":"ok"}` only after success. All other routes are fixed sanitized `503` before readiness. Successful `/api/release-readiness` responses reuse the exact cached `ReleaseReadiness` object while retaining the current shared-admission check; they do not rebuild DuckDB on each request. Request operations still require and prefer the incoming `x-vercel-oidc-token`; only the background initial readiness operation uses the startup fallback.

The live proof sequence must therefore begin only after a separately authorized rebuilt deployment accepts TCP and `GET /health` reaches `200`. A `503 initializing` is expected transient liveness, a `503 failed` is terminal for that process, and neither authorizes corpus execution. The no-retry protected provider probe should run only after this explicit health precondition; it must not treat early `503` responses as proof or hide them with retries. The corrected repository work does not redeploy, contact Vercel, inspect credentials, or establish new live Browser, provider timing, lifecycle, accounting, or memory evidence. A new exact source/artifact pair and fresh live proof are required, while the Hobby peak-memory and Deployment Attestation limitations remain unchanged.
