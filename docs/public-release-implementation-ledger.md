...
The project owns a narrow `HttpTransport` protocol implemented with bounded `requests` streaming. It does not import `vercel._internal.*` and does not claim that the current public Python SDK supplies CAS. An authenticated private `GET` targets the internally derived object URL with exactly `?cache=0`, bearer Authorization, JSON Accept, a five-second timeout, and redirects disabled. It does not claim uncached semantics from request `Cache-Control` or invented Blob cache headers. Writes pin the first-party `@vercel/blob` 2.4.2 v12 shape: `PUT https://vercel.com/api/blob/?pathname=<percent-encoded-pathname>`, bearer Authorization, `x-api-version: 12`, normalized bare `x-vercel-blob-store-id`, `store-id:unix-ms:random-hex` request ID, attempt `0`, private access, JSON content type, and no random suffix. Existing-object writes use the strictly validated provider-compatible conditional tag in `x-if-match` with `x-allow-overwrite: 1`: exact quoted strong tags are preserved, and exact uppercase weak `W/"opaque"` is translated to quoted `"opaque"`. Create-only writes use `x-allow-overwrite: 0` and never send `x-if-match`. HTTP 412 is a precondition conflict. HTTP 401, 403, 429, and 5xx responses are provider errors and are never create/CAS conflicts. Writes are never automatically retried: an ambiguous outcome fails closed, and only a later coordinator attempt after a fresh read may write again.
...
For provider runtime only, startup validates local configuration and constructs the Blob Adapter without provider I/O before yielding. One process-wide background initializer then performs and caches the exact heavyweight local Release Bundle/DuckDB readiness exactly once; it never invokes shared admission or any provider operation. This prompt lifespan yield makes the process TCP-invocable; it does not make the application ready. `GET /health` is nonblocking: it returns `503 {"status":"initializing"}` until that process-local work succeeds, permanently becomes fixed sanitized `503 {"status":"failed"}` on local failure, and becomes `200 {"status":"ok"}` only after local success. Health is not Blob-readiness evidence. A non-health request arriving during local initialization now waits on the initializer's one shared completion signal for at most 30 seconds, a conservative bound above the observed 13.6-second cold initialization. It proceeds only when the exact cached `ReleaseReadiness` is ready; timeout, cancellation, or failed completion returns the fixed sanitized `503` without cancelling or restarting the initializer. Request body/CORS handling, request-scoped OIDC binding, shared Blob admission, child execution, and the separate ten-second Query Run deadline remain after this local-readiness gate. `/api/release-readiness` reuses the cached `ReleaseReadiness` object but, through a real protected request, still performs shared-admission readiness with the incoming `x-vercel-oidc-token`; Blob failure returns fixed `503`, while success is the live request-scoped Blob-readiness proof. Public Query Runs retain request-scoped read/CAS admission and fail closed before child execution when the header or Blob is unavailable.
...
Supplied live evidence for source `7e4f82b3f2b43df4491956c6be4778b27e6d09f9`, direct-child artifact `e143845f4a1b90a298a6b0f66bbbab5340bbe775`, bundle digest `ca7bdf82ac88b924a0ddec2d3c4c31948e55c672fb45d12f7222932c43172871`, and protected deployment `dpl_HvQLRzekfXt34TkGF4k9EkjGGdWi` proved prompt TCP startup and one warmed invocation: health transitioned from initializing to `200` after 13.6 seconds, then request-scoped release readiness, root HTML, and capabilities succeeded on that warmed path. A direct mobile Browser subsequently received the raw fixed service-unavailable JSON from a different cold runtime instance. The screenshot is authoritative evidence that operator warming does not prove the first real Browser path. That candidate is therefore historical and insufficient for application readiness even though it built and deployed.

The bounded cold-request correction uses strict red/green HTTP slices for root HTML, capabilities, concurrent coalescing, failed completion, request timeout with later recovery, nonblocking health, downstream body/admission/OIDC/deadline ordering, and cancellation/shutdown cleanup. Application readiness is intentionally stronger than process invocability or local health: on the exact protected source/artifact/bundle candidate, the real Browser/request path must obtain root HTML, capabilities, and request-scoped release readiness successfully. This repository-only correction performs no provider call, credential access, deployment, push, or PR operation; its rebuilt candidate still requires separately authorized live validation.

Final source verification passed `176` focused startup/request-OIDC/canonical-Blob-host/strict-provider tests, `588` fast Python tests with `3` deselected, `3` marked Release Proof tests with `588` deselected, deterministic eval `26/26`, and Coverage Report validators `10/10`. All three generators, canonical runtime/policy and proof-manifest checks, Ruff lint and format over `121` Python files, mypy over `68` source files, web tests `25/25`, the `113`-module production build, configured pre-commit, and diff checks passed. The only warning remained the upstream Starlette/httpx TestClient deprecation. Pi exposed no subagent/review-agent facility, so direct review checked one completion signal, one initializer, state-before-signal ordering, exact cached-readiness gating, bounded timeout and cancellation isolation, failure secrecy, middleware/deadline order, nonblocking health, shutdown task cleanup, and the unchanged request-only OIDC/Blob admission path.

Supplied live evidence for source `033cc01391ab3ca4db661e1a1837f57586249a43`, direct-child artifact `6a9f8f99c49cdbedf01327e050b451a2b144ca43`, bundle digest `ba180aed0e44202d01c5cc36786c04d140b9679b7248f5097a4c8f2955d5e982`, and protected deployment `dpl_A6LMSbEbfgRQqSJ7RS4TfKKgv7ke` proved the bounded cold-Browser correction: a fresh direct Camofox Browser reached the clean protected root and rendered Ground Ball, while exact-candidate release readiness and capabilities returned 200. The same Browser submitted built-in `40-40` twice; both attempts rendered the exact public ten-second timeout, and provider logs recorded both Query Run POSTs as 503. Local decomposition measured 0.677 seconds for Bundle checking, 2.904 seconds for full runtime load/verification, 1.343 seconds for the query, and 3.614 seconds for the standalone worker on the Mac. The repeated fresh child CSV rebuild, schema/row/key verification, and complete row fingerprint pass—not application-shell readiness—is therefore the fifth repository blocker. This supplied evidence is historical for the superseded candidate and does not prove the correction live.

Independent review rejected the first cache correction because a same-UID provider process could still alter writable Bundle payloads and `/tmp` cache objects, and because lstat/hash/path-open sequencing did not bind DuckDB to the hashed inode. The corrected `Dockerfile.vercel` now performs the complete exact Release Bundle, Coverage Report, canonical runtime, source-fingerprint, and relation verification as root during image assembly. A portable lock plus controlled empty staging tree atomically publishes exactly `/app/provider-runtime-cache/pointer.json` and one metadata-digest directory containing `metadata.json` and `runtime.duckdb`; idempotent/concurrent calls validate the one complete object, foreign entries fail closed, and failed builds remove partial trees. The final image makes Bundle/config/cache files mode `0444`, their directories `0555`, keeps them owned by root, sets `PYTHONDONTWRITEBYTECODE=1`, and runs both app and child workers as dedicated UID/GID `10001`. Runtime startup never creates, rebuilds, accumulates, or garbage-collects a cache.

Every retained cache construction or mutation entry point is root-only by a shared effective-UID guard requiring literal UID 0. The image builder checks that guard before imports that can read configuration, Bundle inspection, runtime reconstruction, DuckDB work, timing, environment changes, or filesystem mutation; the cache builder, materializer, database copier, build lock, removal, new-file writer, and fsync helper repeat the guard before touching their inputs. The candidate image proof directly invokes all eight entry points as UID 10001 against UID-owned `/tmp` and requires exceptions with zero created, removed, or modified files, unchanged immutable cache bytes, and no new `/tmp` DuckDB.

Provider mode accepts only the fixed `/app/release-bundle` and `/app/release-config/protected-preview-runtime.json` paths. Initialization fully rechecks the exact Bundle before readiness and activates only the fixed root-owned pointer; the inherited cache environment value is absent before activation and can become only the validated 64-hex metadata digest. Pointer/metadata/database reads reject symlink, owner, mode, link-count, size, hash, schema, identity, extra-entry, and exact-relation drift. File-descriptor and path stats bind device, inode, UID/GID, mode, link count, size, mtime, and ctime before/after hashing and read-only DuckDB open; root ownership plus UID 10001 is the actual same-UID immutability boundary. Every worker eagerly activates the lru-cached read-only runtime before parsing supported or unsupported input, and any activation failure emits one sanitized failed outcome. Canonical v3 smoke evidence separately reports truthful image-build preparation, runtime activation/validation, and worker wall time and requires finite nonnegative timing with `worker_seconds < 10`, exact candidate identities, exact Coverage proof/source fingerprints, exact columns/counts, and the literal ordered six 40-40 rows. Recipe, plan, rows, result envelope, pagination, verification, QueryEvidence, source records, and expected Coverage identity use recursive exact-JSON-type comparison, so Python's bool/int and int/float equality aliases cannot satisfy any semantic gate; only the intentionally numeric timing fields accept either integer or float values, while rejecting booleans and nonfinite, negative, or over-deadline values. Candidate assembly semantically validates those bytes against the sibling Coverage Report before the deterministic-parity gate can pass. The subprocess hard stop, exact deadline, fixed argv, kill/reap behavior, cold-request wait, admission/OIDC/accounting contracts, and request/result envelopes remain unchanged. This repository pass contacts no provider, Browser, credentials, Blob state, deployment, or account and does not push or open a PR.

The independent-review correction followed strict red/green slices for canonical smoke mutations, fixed pointer/root/object inventories, metadata/database/owner/mode/link/symlink/rehashed-relation drift, descriptor/path state, concurrent first publication, idempotent reruns, pre- and post-publication cleanup, fixed provider paths, eager supported/unsupported worker activation, sanitized failure, no runtime rebuild, candidate semantic validation, protected-provider gate derivation, Docker UID/mode contracts, and unchanged local/full verification. Before source freeze, all focused cache/initialization/worker/candidate/provider/release/workflow suites passed; the final full fast suite with branch coverage passed `628` with `4` Release Proof tests deselected and aggregate coverage `74%`; all `4` marked Release Proof tests passed with `628` deselected. Eval passed `26/26`; web Browser contracts passed `25/25`; the production build transformed `113` modules; all generators, Coverage validation, canonical policy/runtime/proof-manifest checks, YAML/diff checks, Ruff lint and format over `125` Python files, mypy over `70` source files, and configured pre-commit passed. The only warning remained the upstream Starlette/httpx TestClient deprecation. Pi exposed no subagent/review-agent facility, so direct self-review checked the root/non-root threat boundary, atomic publication and cleanup, portable collision handling, fixed paths, strict owner/mode/hash/size/link/inode/ctime/relation checks, read-only DuckDB use, full Bundle-before-readiness behavior, source/Bundle/runtime/Coverage binding, evidence false-pass mutations, eager lru worker loading, no fallback or `/tmp` accumulation, subprocess sanitization/reaping, unchanged admission/deadline/accounting/OIDC contracts, and absence of external mutation.

Supplied follow-up evidence then exposed a provider-identity defect before any repository-valid proof could be retained: exact Vercel deployment ID `dpl_9XW9KmE2rqe4XWZ7YBbmetEQLgab` was rejected because `deployment_id` incorrectly used the lowercase-only generic canonical-ID rule. The correction adds one shared exact Vercel deployment validator for `dpl_` plus the current 28-character case-sensitive base62 suffix observed consistently in first-party deployment IDs. Provider evidence, protected Blob/HTTP probes, lifecycle replacement evidence, aggregate identity binding, and Vercel Deployment Attestation now validate and preserve the exact value without lowercasing, aliasing, truncation, or substitution; malformed identifiers and case-mutated candidate bindings fail closed. Strict TDD recorded `22` expected failures against the old behavior plus one focused pre-transport probe red, then the provider-proof/Blob/probe/candidate/cache focus passed `133`. Final source validation passed `643` fast Python tests with `4` Release Proof tests deselected, all `4` Release Proof tests with `643` deselected, deterministic eval `26/26`, Coverage validators `10/10`, web tests `25/25`, and the `113`-module production build. All generators, canonical policy/proof-manifest checks, Ruff lint and format over `126` Python files, and mypy over `71` source files passed. The upstream Starlette/httpx TestClient deprecation remains the only warning. This repository correction made no provider, Browser, credential, deployment, push, or PR call.

A second independent review found that Python equality still allowed bool/int and int/float aliases throughout schema-v3 smoke semantics, and that retained builder helpers could be directly applied by UID 10001 to UID-owned `/tmp`. Strict TDD first reproduced both classes: exact canonical-byte, mapping-validator, producer, and candidate semantic-gate matrices accepted type-mutated recipes, plans, rows, envelope counts, pagination, bindings, QueryEvidence counts, and source row counts; direct builder/materializer/copy/lock/remove/write/fsync probes read inputs or changed state before denial. The correction uses recursive exact-JSON-type comparison for every query-proof value while preserving explicit finite nonnegative numeric timing acceptance, and centralizes a literal effective-UID-0 guard as the first operation of every retained filesystem mutation entry point. Source/workflow contracts pin every guard and the network-disabled UID 10001 direct-probe inventory. Focused cache/smoke/candidate/provider-init/execution/release/workflow validation passed `280` tests after the red phase; the complete fast suite passed `748` with `4` Release Proof tests deselected, all `4` Release Proof tests passed with `748` deselected, Coverage validators passed `10/10`, deterministic eval passed `26/26`, and web tests passed `25/25` with a `113`-module production build. All generators, canonical config/manifest/Bundle checks, Ruff lint/format over `127` Python files, mypy over `76` source files, configured pre-commit, YAML, and diff checks passed. The only warning remained the upstream Starlette/httpx TestClient deprecation. Pi exposed no subagent/review-agent facility, so direct self-review checked exact-type recursion at every schema-v3 branch, unchanged timing semantics, first-operation guard placement, nested guarded calls, root cleanup and concurrency behavior, UID 10001 input-read and side-effect denial, unchanged provider boundary/config/Bundle behavior, and preservation of deployment identity, OIDC, admission, and hard-stop contracts. No provider, Browser, credentials, deployment, push, or PR operation occurred.

The subsequent live candidate-bound Blob evidence exposed a content-addressed ETag defect: rewriting identical admission-state bytes did not prove a version change, so the tested stale conditional write could succeed and `bounded_contention` failed even though the command exited zero. Strict TDD reproduced that behavior with a content-addressed-ETag transport. The corrected conflict transport strictly decodes each current state, preserves its budget and Visitor histories, rejects malformed/full/colliding state, replaces only its own canonical ancient expired marker with a fresh unique reserved marker, re-encodes canonically, and requires byte-distinct content plus a successful competing write. It does not require or trust a write-response ETag; instead it performs a fresh private uncached read that must return the exact competing bytes with a present, valid ETag distinct from the pre-write ETag. Every original write must then return `412`; failed or non-200 re-reads, missing/malformed/unchanged ETags, mismatched bodies, competing errors, or unexpected original success fail closed with the sanitized Blob error. All eight coordinator retries now conflict and terminate exactly as `provider_unavailable` / `coordination_contention`. The CLI writes canonical pass or failure evidence for audit and returns zero only for `status: pass`. Focused Blob/CAS/state/provider-proof/candidate/workflow validation passed `256` tests; the complete fast suite with branch coverage passed `764` with `4` Release Proof tests deselected at `75%` aggregate coverage; all `4` Release Proof tests passed with `764` deselected; Coverage validators passed `10/10`; deterministic eval passed `26/26`; and web tests passed `25/25` with a `113`-module production build. No provider, Browser, credential, deployment, push, or PR operation occurred.
