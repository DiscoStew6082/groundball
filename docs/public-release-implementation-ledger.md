...
The repository correction preserves the subprocess hard stop and exact ten-second deadline. After full `release_readiness()` has verified the exact Bundle, Coverage Report, canonical in-memory DuckDB relations, rows, keys, and source fingerprints, protected background initialization copies that verified database into one private staging directory under the fixed `/tmp/groundball-provider-runtime-cache-v1` namespace. Canonical sidecar metadata binds the source commit, Bundle digest, protected runtime-configuration digest, data release, semantic timestamp-free data manifest, source fingerprints, exact required relation inventory, database byte size, and database SHA-256. The staging directory is atomically renamed only after both regular owned files are complete and changed to read-only minimal modes; initialization then revalidates the final cache and publishes one 64-hex metadata digest in `GROUNDBALL_PROVIDER_RUNTIME_CACHE`. Failure clears the reference and permanently fails that process's one-shot initialization.

A protected child accepts no cache path. It derives fixed filenames only from that digest under the fixed namespace, rejects missing, malformed, unknown, stale, foreign, symlinked, writable, wrong-owner, wrong-mode, partial, resized, rehashed, or relation-inexact state, verifies the database digest before opening DuckDB read-only, and checks the file did not change while opening. Source fingerprints and data release come only from the strict parent sidecar bound by that digest and the database digest. The child skips full Bundle payload verification, CSV import, schema/row/key scans, and fingerprint recomputation; local CI, ordinary development, and no-cache Release Proof continue through the original full loader. The candidate-container workflow now runs an additional network-disabled protected-runtime-config smoke that prepares the cache and invokes the real `40-40` subprocess worker. This repository pass contacts no provider, Browser, credentials, Blob state, deployment, or account and does not push or open a PR.

Strict red/green slices covered normal-to-cache `40-40` parity, structured and Retrosheet parity, absent expensive child functions, immutable read-only behavior, strict identity/metadata/file rejection, missing-cache fail-closed behavior, partial-build cleanup, provider initialization ordering, sanitized child failure, unchanged local full verification, and the assembled provider-mode worker smoke. Before source freeze, `91` focused affected tests passed; the fast Python suite passed `610` with `4` Release Proof tests deselected; all `4` marked Release Proof tests passed with `610` deselected; eval passed `26/26`; Coverage Report validators passed `10/10`; all generators and canonical config/manifest checks passed. Ruff lint and format passed over `125` Python files, mypy passed over `75` source files, web tests passed `25/25`, the production build transformed `113` modules, configured pre-commit passed, and diff checks passed. A local assembled-bundle provider smoke measured `5.004407` seconds for full readiness plus cache preparation and `0.106245` seconds for the real cached `40-40` child; the proof asserts completion inside the unchanged ten-second runner rather than a universal wall-clock threshold. The only warning remained the upstream Starlette/httpx TestClient deprecation. Pi exposed no subagent/review-agent facility, so direct self-review checked atomic publication, fixed-path derivation, strict ownership/mode/hash/size/relation checks, timestamp-free identity, read-only DuckDB use, source/Bundle/runtime binding, no fallback, subprocess sanitization/reaping, unchanged admission/deadline accounting, local/Release Proof isolation, and absence of external mutation.
