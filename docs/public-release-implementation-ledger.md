...
- The Coverage Report still proves `5,253/5,253` obligations with zero uncovered; its compiler identity and proof ID were regenerated canonically. Catalog compatibility, raw inventory, coverage generation check/validator, full Ruff, changed-Python format check, mypy, web build, configured pre-commit, and `git diff --check` are required again after final Release Bundle assembly.
