...
Supported stat-definition questions such as "what is OPS?" read the checked-in local stat-definition Markdown and return stat-definition provenance. Broader baseball explanation questions remain open LLM answers. No explanation path uses a vector index at runtime.
