...
`question_interpretation.py` owns the model prompt, closed response schema, validation, and `run_question_input`. The existing deterministic adapter runs first. When injected interpretation is available, it may propose typed statistics slots, a published Query Recipe, a bounded research request or compound plan, a clarification code, or rejection. `query_intent.py` translates slots using the catalog; complex capabilities retain the direct recipe path. It cannot supply factual prose or SQL. Proposed recipes still pass the same planner and compiler.

`assistant.py` owns factual selection and rendering for the topics enumerated in `RESEARCH_TOPICS`, including current leaders, probable pitchers and World Series follow-ups. Statistical facts retain their immutable Query Run and QueryEvidence. `ResearchSources` supplies bounded fixture, historical postseason and optional current MLB records; public code validates their scope and constructs the answer. The outer runtime owns model transport and external I/O, while the public repository remains the source of product behavior.
...
Fixture metadata identifies the next listed upcoming game, not a complete schedule. Statistical history ends in 2025; World Series context requires supporting records. Ten reviewed definitions link to their individual MLB glossary references. Current-source leaderboards and tentative probable pitchers remain separate from the historical query catalog. Compound plans use at most one query and refuse incomplete results. Follow-ups pass validated references and retrieve evidence anew. Unsupported conditions must be rejected or clarified, never removed to produce a generic answer. See [assistant.md](assistant.md).
